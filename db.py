...
def start():
    ...
    try:
        ...
        c.execute("ALTER TABLE meme_state ADD meme_rating INTEGER")
        c.execute("ALTER TABLE meme_state ADD rating_count INTEGER")
...
def set_option(name: str, value: str):
    cursor().execute("insert into options values(?, ?) on conflict(name) do update set value=excluded.value",
                     (name, value))
    commit()
