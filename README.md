...
- Important: You will need to create a `.env` file and inside it add your bot token, and the 'default guild' IDs. It will need to be like: `BOT_TOKEN=<insert your token here>` and `DEFAULT_GUILDS = <insert your guild ID(s) here>`. Specifying the guild ID to the bot isn't neccessary, but specifying certain IDs to it (can be many guilds) means that slash commands will only appear in those guild(s). Btw, 'guild' means a Discord server. 
