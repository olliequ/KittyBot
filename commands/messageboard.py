...
async def show_message_stats(ctx: lightbulb.Context, plot_type, set_num) -> None:
    ...
    if plot_type == 1:
        ...
        message = [
            "**Messages Tally from {} to {}** :cat:```".format(
                set_num * 10 + 1, set_num * 10 + 10
            )
        ]
        for name, count, _ in users_counts:
            ...
            if slices % SLICES_PER_CHAR > 0:
                bar += chr(
                    BLOCK_CODEPOINT + SLICES_PER_CHAR - (slices % SLICES_PER_CHAR)
                )
    ...
    # Light mode graph.
        ...
        ax.set_title(
            "Messages Tally! from {} to {}".format(set_num * 10 + 1, set_num * 10 + 10),
            pad=15,
            color="#333333",
            weight="bold",
            fontsize="15",
